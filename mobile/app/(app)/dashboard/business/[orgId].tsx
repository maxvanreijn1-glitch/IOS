...
import { apiGetBookings, apiAcceptBooking, apiCancelBooking, apiGetOrgMembers, apiGetOrgClients } from '@/lib/api';
...
export default function OrgAdminScreen() {
  ...
  const isOwner = membership?.role === 'OWNER' || user?.ownedOrganisations.some((o) => o.id === orgId);
  const isMember = membership?.role === 'MEMBER';

  useEffect(() => {
    const org =
      user?.ownedOrganisations.find((o) => o.id === orgId) ??
      user?.memberships.find((m) => m.organisationId === orgId);
    if (org) {
      setOrgName('name' in org ? org.name : (org as any).organisationName ?? '');
    }
  }, [orgId, user]);

  const loadData = useCallback(async () => {
    if (!orgId) return;
    const [b, members, clients] = await Promise.all([
      apiGetBookings(orgId),
      apiGetOrgMembers(orgId),
      apiGetOrgClients(orgId),
    ]);
    setBookings(b);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        {past.length > 0 && (
          <>
            <Text style={[styles.sectionTitle, { marginTop: 20 }]}>Past / Cancelled</Text>
            {past.map((b) => (
              <BookingCard
                key={b.id}
                booking={b}
...
function BookingCard({
  ...
  const canAccept = booking.status === 'PENDING' && (booking.memberId === currentUserId || booking.clientId === currentUserId);
  const canCancel = booking.status !== 'CANCELLED' && (isOwner || booking.memberId === currentUserId || booking.clientId === currentUserId);
  ...
  return (
    <View style={styles.bookingCard}>
      ...
      <Text style={styles.bookingPeople}>
        Client: {booking.client.name ?? booking.client.email} · Staff: {booking.member.name ?? booking.member.email}
      </Text>
      {(canAccept || canCancel) && (
